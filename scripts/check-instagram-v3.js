...
console.log('🚀 Instagram Checker v3 (RSS Method)');
...
async function checkInstagram(username, webhookUrl) {
  ...
  try {
    // Use RSS Bridge (public instance) to get Instagram feed
    ...
    // Fallback: Send test notification on first run
      ...
      try {
        await axios.post(webhookUrl, {
          ...
          embeds: [{
            ...
            footer: { text: 'Initial Setup Confirmation' }
          }]
        });
